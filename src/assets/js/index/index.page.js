function loadTecmImagesPopUp() {
	...
	tech_images.forEach((el) => {
		el.addEventListener("click", (ev) => {
			Swal.fire({
				title: `<strong>${el.dataset.alert_title ? el.dataset.alert_title : ""
					}<\strong>`,
				html: `<img src="${el.src ? el.src : "#"
					}" style="max-width: 128px;"/> <br /> <p class="mt-4">${el.dataset.alert_text ? el.dataset.alert_text : ""
					}</p>`,
			});
		});
	});
